...
- JWT 認証を使用する。
- Jinja2 画面との相性を考慮し、JWT は HttpOnly Cookie で保持する方針を推奨する。
...
AND current_timestamp >= users.valid_from
AND current_timestamp <= users.valid_to
```
...
| Method | Path | 権限 | 概要 |
| --- | --- | --- | --- |
| POST | `/api/auth/login` | 未ログイン | JWT発行、ログイン成功/失敗ログ記録 |
| POST | `/api/auth/logout` | ログイン済み | JWT破棄、ログアウトログ記録 |
...
4. `activity_logs` に `photo_download` 登録
...
- `login_id` 重複をチェック。
- `activity_logs` に `user_create` を記録。
...
4. サムネイルを生成する。
5. 原本とサムネイルを R2 に保存する。
6. `photos` を登録する。
7. `activity_logs` に `photo_upload` を記録する。
...
ダウンロード API では、Presigned URL を返す前に `download_histories` と `activity_logs` の両方へ記録します。
